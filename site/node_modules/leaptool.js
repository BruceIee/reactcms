...
exports.getJsonFromText = function(inputText) {
    var jsonResult = null;
    if (inputText) {
        try {
...
exports.toArray = function(input, delim) {
    var result = [];
    // try input is stringified JSON array, otherwise convert as string
    try {
        ...
        result = exports.convertStringToArray(input, delim);
    }
    return result;
};
...
exports.convertStringToArray = function(input, delim) {
    var result = [];
    delim = delim || ',';
    if (input && input.constructor.name != 'Array') {
        var taglist = input.split(delim);
        for (var i in taglist) {
            result.push(taglist[i].trim());
        }
    } else {
        result = input;
    }
...
// parse JSON input and supress exception
exports.JsonParse = function(input) {
    var result = null;
    if (input) {
        try {
            result = JSON.parse(input);
        } catch(e) {
            console.log('JSON parse input', input, '\nerror:', e);
        }
    }
    return result;
};

exports.getFieldDefault = function(fieldType) {
    var value = '';
    switch (fieldType) {
        case 'string':
            value = '';
...
// normalize input with model data
exports.normalizeModelInput = function(input, moduleName, model) {
    for (var fieldName in model) {
        
        console.log('>>> fieldName:', fieldName, input[fieldName]);
        
        var field = model[fieldName];
        var inputValue = input[fieldName] || exports.getFieldDefault(field.type);
        if (field.type === 'file') {
            console.log('>>> file type', inputValue && inputValue.length);
            if (inputValue && inputValue.length > 0) {
                input[fieldName] = inputValue[0];
...
exports.normalizeModelInput2 = function(input, moduleName, moduleModel) {
    var output = { _class:moduleName };
    for (var fieldName in model) {
